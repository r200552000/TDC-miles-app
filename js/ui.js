...
function renderSettingsCards() {
    ...
    if (!db.settings.billingDays) db.settings.billingDays = {};
        ...
        BUILTIN_DEFS.forEach(def => {
            if((db.settings.enabledBuiltins || []).includes(def.id)) {
                if(def.id === 'taishin_cx') {
                    ...
                    let targetId = def.id;
                    if (['hsbc_live_selected', 'hsbc_live_asia', 'hsbc_live_task'].includes(def.id)) {
                        targetId = 'hsbc_live';
                    }
                    let currentDay = db.settings.billingDays[targetId] || db.settings.billingDays[def.id.split('_')[0]] || 1;
                    bHtml += `<div class="tdc-flex tdc-justify-between tdc-align-center tdc-mb-2"><span class="small fw-bold text-dark">${def.name}</span><select class="form-select form-select-sm w-auto rounded-pill" style="border-width: 2px;" onchange="updateBillingDay('${targetId}', this.value)">${Array.from({length:31}, (_, i)=>`<option value="${i+1}" ${i+1===currentDay?'selected':''}>每月 ${i+1} 日</option>`).join('')}</select></div>`;
                }
            }
        });
...
function addCustomCard() {
    ...
    const db = loadDB();
    if (!db.customCards) db.customCards = [];
...
function saveSettings() {
    ...
    if(elGemini) {
        ...
        if(key) safeSetItem('GEMINI_API_KEY', key); else safeRemoveItem('GEMINI_API_KEY');
    }
    if(elGithub) {
        const git = elGithub.value.trim();
        if(git) safeSetItem('GITHUB_GIST_TOKEN', git); else safeRemoveItem('GITHUB_GIST_TOKEN');
    }
