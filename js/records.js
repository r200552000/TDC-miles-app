...
function recordTx(idx, isManual = false, originalMiles = 0) {
    ...
    let unitText = '哩';
    let points = item.miles;
    if (item.id === 'hsbc_live') {
        points = Math.trunc(item.miles / 2);
        unitText = '積分';
    } else if (item.id === 'hsbc_inf') {
        unitText = '積分';
    }
    ...
    const consumedQuotaMap = item.consumedQuotaMap || null;
    ...
    // Live+ 三池：分別寫入
        ...
        Object.entries(consumedQuotaMap).forEach(([subId, amount]) => {
            if (!amount) return;
            const subLimitKey = getLimitKey(db, subId, now);
            if (!db.limits[subLimitKey]) db.limits[subLimitKey] = { spend: 0, points: 0 };
...
function openManualAdjust(idx) {
    const item = currentResults[idx];
    document.getElementById('manual-adjust-val').value = item.miles;
    document.getElementById('manual-adjust-idx').value = idx;
    showModal('manualAdjustModal');
}

function confirmManualAdjust() {
    const idx = document.getElementById('manual-adjust-idx').value;
    const manualVal = parseFloat(document.getElementById('manual-adjust-val').value);
    if (isNaN(manualVal)) return showCustomAlert('請輸入有效的數字！');
    const item = currentResults[idx];
    const originalMiles = item.miles;
    item.miles = manualVal;
    hideModal('manualAdjustModal');
    recordTx(idx, true, originalMiles);
}
...
function renderStatus() {
    ...
    if (Object.keys(agg).length === 0) {
        statusCon.innerHTML = '<div class="tdc-text-center py-3 text-muted font-hand fw-bold">本月尚無累積</div>';
    } else {
        let h = '';
        for (let id in agg) {
            const item = agg[id];
            h += `
            <div class="tdc-flex tdc-justify-between tdc-align-center tdc-mb-3 pb-3 border-bottom">
                <div>
                    <div class="fw-bold text-dark tdc-mb-1">${item.name}</div>
                    <div class="text-muted small">$${item.spend.toLocaleString()}</div>
                </div>
                <div class="text-primary fw-bold fs-5">${item.miles.toLocaleString()} <small class="text-muted" style="font-size:0.6em">${item.unit}</small></div>
            </div>`;
        }
    ...
    (db.settings.enabledBuiltins || []).forEach(id => {
        // Live+ 改成三池獨立顯示，不走一般卡流程
        if (id === 'hsbc_live') {
            const liveDefs = [
                { subId: 'hsbc_live_selected', label: '匯豐 Live+｜三大通路 3%' },
                { subId: 'hsbc_live_asia', label: '匯豐 Live+｜亞洲七國餐飲 1%' },
                { subId: 'hsbc_live_task', label: '匯豐 Live+｜自動扣繳 1%' }
            ];

            liveDefs.forEach(def => {
                const limit = getLimitVal(def.subId);
                const lKey = getLimitKey(db, def.subId, now);
                const curr = db.limits[lKey] ? db.limits[lKey].spend : 0;
                const pct = Math.min(100, Math.max(0, (curr / limit) * 100));
                ...
                <div class="tdc-mb-3">
                    <div class="tdc-flex tdc-justify-between tdc-align-center small tdc-mb-1">
                        ...
                        <div class="tdc-flex tdc-align-center gap-2">
                            <span>$${curr.toLocaleString()} / $${limit.toLocaleString()}</span>
                            <button class="btn-edit-limit" title="修改進度" onclick="openEditLimit('${def.subId}')">
                                <svg class="lucide"><use href="#icon-pen"/></svg>
                            </button>
                        </div>
                    </div>
        ...
        const limit = getLimitVal(id);
        if (limit >= 999999999) return;
        ...
        <div class="tdc-mb-3">
            <div class="tdc-flex tdc-justify-between tdc-align-center small tdc-mb-1">
                ...
                <div class="tdc-flex tdc-align-center gap-2">
                    <span>$${curr.toLocaleString()} / $${limit.toLocaleString()}</span>
                    <button class="btn-edit-limit" title="修改進度" onclick="openEditLimit('${id}')">
                        <svg class="lucide"><use href="#icon-pen"/></svg>
                    </button>
                </div>
            </div>
        ...
        if (c.limitAmt && c.limitAmt > 0) {
            ...
            <div class="tdc-mb-3">
                <div class="tdc-flex tdc-justify-between tdc-align-center small tdc-mb-1">
                    ...
                    <div class="tdc-flex tdc-align-center gap-2">
                        <span>$${curr.toLocaleString()} / $${c.limitAmt.toLocaleString()}</span>
                        <button class="btn-edit-limit" title="修改進度" onclick="openEditLimit('${c.id}')">
                            <svg class="lucide"><use href="#icon-pen"/></svg>
                        </button>
                    </div>
                </div>
            ...
            <div class="tdc-mb-3">
                <div class="tdc-flex tdc-justify-between tdc-align-center small tdc-mb-1">
                    ...
                    <div class="tdc-flex tdc-align-center gap-2">
                        <span>已刷 $${curr.toLocaleString()}</span>
                        <button class="btn-edit-limit" title="修改進度" onclick="openEditLimit('${c.id}')">
                            <svg class="lucide"><use href="#icon-pen"/></svg>
                        </button>
                    </div>
...
function renderList() {
    ...
    const monthKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
    const data = db.records[monthKey] || {};
    const con = document.getElementById('transaction-list-container');
    con.innerHTML = '';
    ...
    for (let key in data) {
        ...
        groups[safeName].push({ ...r, key });
        count++;
    }

    document.getElementById('nav-badge-list').innerText = count;
    document.getElementById('nav-badge-list').style.display = count > 0 ? 'inline-block' : 'none';
    document.getElementById('pending-count').innerText = `${count} 筆`;

    if (count === 0) {
        con.innerHTML = '<div class="tdc-text-center text-muted py-5 font-hand fw-bold">本月目前沒有待結算的紀錄</div>';
        return;
    }
...
function delTx(monthKey, txId) {
    ...
    const db = loadDB();
    const rec = db.records[monthKey] ? db.records[monthKey][txId] : null;
    if (!(rec && typeof rec === 'object')) return;

    // Live+ 三池退回
    if (rec.consumedQuotaMap) {
        Object.entries(rec.consumedQuotaMap).forEach(([subId, amount]) => {
            if (!amount) return;
            const subLimitKey = getLimitKey(db, subId, new Date(parseInt(txId.split('_')[1] || Date.now())));
